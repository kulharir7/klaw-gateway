...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["klaw security audit", "Run a local security audit."],
          ["klaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["klaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["klaw security audit --json", "Output machine-readable JSON."],
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("klaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("klaw security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("klaw security audit --fix")}`));
        if (!fixResult) {
