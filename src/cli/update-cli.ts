...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Klaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["klaw update", "Update a source checkout (git)"],
        ["klaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["klaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["klaw update --tag beta", "One-off update to a dist-tag or version"],
        ["klaw update --dry-run", "Preview actions without changing anything"],
        ["klaw update --no-restart", "Update without restarting the service"],
        ["klaw update --json", "Output result as JSON"],
        ["klaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["klaw update wizard", "Interactive update wizard"],
        ["klaw --update", "Shorthand for openclaw update"],
      ] as const;
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openclaw.ai/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["klaw update status", "Show channel + version status."],
          ["klaw update status --json", "JSON output."],
          ["klaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
