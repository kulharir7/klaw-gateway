    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["klaw nodes status", "List known nodes with live status."],
          ["klaw nodes pairing pending", "Show pending node pairing requests."],
          ['openclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["klaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
