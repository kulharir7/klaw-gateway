...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("klaw node install"), formatCliCommand("klaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("klaw node install --force")}`);
    }
    return;
  }
